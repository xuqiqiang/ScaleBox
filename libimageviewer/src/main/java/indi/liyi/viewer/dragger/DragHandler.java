...
public class DragHandler {
     ...
     * 手指抬起
     */
    public void onUp(ImageView imageView, ViewData viewData, ImageTransfer.OnTransCallback restoreCallback,
                     ImageTransfer.OnTransCallback exitCallback) {
    ...
    private void transformBgAlpha(int alpha) {
        if (mBackground != null && mBackground.getAlpha() != alpha) {
            ...
            if(mDragListener != null)
                mDragListener.onDrag(alpha / 255f);
        }
    }

    public interface DragListener {
        void onDrag(float alpha);
    }
}
