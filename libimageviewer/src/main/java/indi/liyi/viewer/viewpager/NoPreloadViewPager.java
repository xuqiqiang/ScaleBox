...
//            mAdapter.registerDataSetObserver(mObserver);
    ...
    void setCurrentItemInternal(int item, boolean smoothScroll, boolean always, int velocity) {
        if (DEBUG_TOUCH) Log.i(TAG, "_test_vp_ setCurrentItemInternal:" + item);
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    if (DEBUG_TOUCH) Log.i(TAG, "_test_vp_ onTouchEvent scrollX:" + scrollX + "," + widthWithMargin + "," + currentPage);
                    if (DEBUG_TOUCH) Log.i(TAG, "_test_vp_ onTouchEvent mDownMotionX:" + mDownMotionX + "," + ev.getX());
                    if (DEBUG_TOUCH) Log.i(TAG, "_test_vp_ onTouchEvent dis:"
                            + (Math.abs(ev.getX() - mDownMotionX) < widthWithMargin / 4f)
                            + "," + (System.currentTimeMillis() - mDownMotionTime) + "," + mCurItem);

                    int nextPage;
                    if (Math.abs(ev.getX() - mDownMotionX) < widthWithMargin / 4f
                            && System.currentTimeMillis() - mDownMotionTime > 300
                            || Math.abs(ev.getX() - mDownMotionX) < widthWithMargin / 2f
                            && System.currentTimeMillis() - mDownMotionTime > 600) {
                        nextPage = mCurItem;
                    } else {
                        if (initialVelocity == 0) {
                            nextPage = ev.getX() > mDownMotionX ? mCurItem - 1 : mCurItem + 1;
                        } else {
                            nextPage = initialVelocity > 0 ? currentPage : currentPage + 1;
                        }
                    }
//                    int nextPage = initialVelocity >= 0 ? currentPage : currentPage + 1;
                    if (DEBUG_TOUCH) Log.i(TAG, "_test_vp_ onTouchEvent setCurrentItemInternal:" + nextPage + "," + initialVelocity);
