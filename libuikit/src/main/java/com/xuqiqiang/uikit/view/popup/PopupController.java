...
class PopupController {
    ...
    void setBackGroundLevel(float level) {
        setBackGroundLevel(Color.BLACK, level);
    }
     ...
     */
    void setBackGroundLevel(int color, float level) {
//        Log.d("setBackGroundLevel:" + level);
        if (context instanceof BaseAppCompatActivity) {
            ((BaseAppCompatActivity) context).setDimAmount(color, level);
        } else {
            mWindow = ((Activity) context).getWindow();
            WindowManager.LayoutParams params = mWindow.getAttributes();
            params.alpha = 1 - level;
...
//            if (isShowBg) {
//                controller.setBackGroundLevel(bgColor, bg_level);
//            }
            if (isShowAnim) {
